...
"""
일기 텍스트를 분석해 감정(단일 라벨) + 상황 맞춤 음악 1곡을 추천하는 모듈.
...
def analyze_text(text: str, genres: Optional[List[str]] = None) -> dict:
    ...
    - music.reason 값은 '문장만' 작성하며, "일기와의 연결 이유 설명:" 같은 라벨/접두사는 절대 붙이지 말 것.
    """
    ...
    try:
        ...
        # 장르 정규화(항상 main/sub 보장)
        g_raw = music.get("genre", "") or f"{music.get('genre_main','')}/{music.get('genre_sub','')}"
        main, sub, fused = _ensure_major_sub(g_raw, preferred_majors)
        music["genre_main"], music["genre_sub"], music["genre"] = main, sub, fused

        title = music.get("title", "") or ""
        reason_blob = f"{music.get('reason','')} {music.get('track_summary','')}"
        tempo = music.get("tempo", "") or ""
        conflict, why = _theme_conflict(text, title, reason_blob, music.get("track_summary","") or "", tempo, fused)

        # 선호 대분류 보정 필요?
        prefer_major_fix = (preferred_majors and score < PREFERRED_FORCE_THRESHOLD and main not in preferred_majors)

        if conflict or not sub or prefer_major_fix:
            fixes: List[str] = []
            if conflict and why == "romance_theme_without_diary_context":
                fixes.append("연애/사랑/실연 테마 금지(일기에 해당 단서 없음)")
            if conflict and why in ("too_fast_for_study", "too_energetic_for_study"):
                fixes.append("공부/집중 맥락에 맞춰 저가사·저에너지로 조정")
            if not sub:
                fixes.append("장르는 반드시 '대분류/소분류'로 지정")
            if prefer_major_fix:
                fixes.append(f"대분류는 선호 집합({', '.join(preferred_majors)}) 중 하나로 선택")
                # 컨텍스트 힌트 재주입(참고용)
                fixes.append(_build_subgenre_hint(preferred_majors, contexts))

            repair_prompt = f"""
            다음 문제를 수정하여 동일 JSON 스키마로 다시 출력하라.
            - 수정 규칙: {", ".join(fixes)}
            - 일기: {text}
            - 선호 대분류(우선 고려): {", ".join(preferred_majors) or "없음"}
            - 현재 감정 점수: {score:.2f}
            - 감지된 컨텍스트: [{contexts_text}]
            """
            data = _call([
        ...
        if isinstance(data.get("music"), dict):
            m = data["music"]
            m.pop("genre_main", None)
            m.pop("genre_sub", None)
