...
def convert_to_wav_16k_mono(input_bytes: bytes, in_ext: str) -> bytes:
    ...
    try:
        ...
        print(f"[DEBUG] ffmpeg input path: {in_path}, size={os.path.getsize(in_path)} bytes")
        print(f"[DEBUG] ffmpeg output path: {out_path}")
...
async def transcribe(
    ...
    """
    print(f"[DEBUG] Uploaded filename: {file.filename}, content_type={file.content_type}")
    ...
    try:
        ...
        if getattr(settings, "SEND_STT_TO_SPRING", False):
            ...
            try:
                ...
                requests.post(
                    settings.SPRING_STT_URL,   # ✅ .env에서 관리
